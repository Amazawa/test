...
infoln "Creating config transaction to add org$ORG_NUM to network"
...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"Org'$ORG_NUM'MSP":.[1]}}}}}' config.json ./organizations/peerOrganizations/org$ORG_NUM.example.com/org$ORG_NUM.json > modified_config.json
{ set +x; } 2>/dev/null

# Compute a config update, based on the differences between config.json and modified_config.json, write it as a transaction to org3_update_in_envelope.pb
createConfigUpdate ${CHANNEL_NAME} config.json modified_config.json org${ORG_NUM}_update_in_envelope.pb
